...
function operationalEntriesForTrips(){
  const activities=(operationalDatabase?.records||[]).filter(r=>r?.recordKind==='activity'),months=new Set();
  activities.forEach(x=>{const m=String(x?.date||'').slice(0,7);if(m)months.add(m)});
  const out=[];
  [...months].sort().forEach(month=>{
    out.push(...operationalEntriesForMonth(month));
  });
