...
function migrateSnapshotToOperationalDatabase(snapshot={}){
  ...
  for(const source of (Array.isArray(snapshot.roster)?snapshot.roster:[])){
    ...
    if(i>=0){
      ...
      for(const key of operationalKeys)if(Object.prototype.hasOwnProperty.call(existing,key))merged[key]=existing[key];
      records[i]=merged;
...
function findRecordForEntry(row,records){
  const byId=records.find(x=>x.id===row.id);
  if(byId)return byId;
  const logtenUid=String(row?.logtenUniqueId||'').trim();
  if(logtenUid){
    return records.find(x=>String(x?.logtenUniqueId||'').trim()===logtenUid)||null;
  }
  return (row.rosterItemId?records.find(x=>x.id===row.rosterItemId||x.legacyRosterId===row.rosterItemId):null)
    ||(row.rosterAerolineKey?records.find(x=>x.aerolineKey===row.rosterAerolineKey||x.rosterAerolineKey===row.rosterAerolineKey):null)
    ||uniqueUnifiedFlightMatch(records,row);
}
...
function setEntryTypeUI(){
  ...
  if(sim){
    const hide=['flightNo','dep','arr','schedOut','schedIn','schedBlock','out','off','on','in','block','role','seatPosition','night','ifr','pf','approachType','delayReason'];
...
function calcEntry(){
  ...
  else if(dutyType==='Simulator')$('calcPreview').textContent=`Simulator ${fmt(c.simulatorTime)} • Credit ${fmt(c.credit)}`;
...
document.addEventListener('DOMContentLoaded',async()=>{
  ...
  if('serviceWorker' in navigator){navigator.serviceWorker.register('./sw-9.10.2.js').catch(err=>console.warn('Offline cache unavailable',err));}
