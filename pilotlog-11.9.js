...
function isCompletedLogbookEntry(f,now=new Date()){
  return (isFlight(f)||isSim(f))&&activityIncludedInResults(f);
}
...
async function aircraftByRegistration(registration){
  ...
  try{
    ...
    if(response.status===404){aircraftRegistrationCache.set(reg,null);return null}
    ...
    if(!aircraft||typeof aircraft!=='object'){aircraftRegistrationCache.set(reg,null);return null}
...
async function applyAircraftRegistrationLookup(){
  ...
  const seq=++aircraftLookupSeq;
  try{
    const aircraft=await aircraftByRegistration(registration);if(seq!==aircraftLookupSeq||!aircraft)return false;
    ...
    if(formManualFields.type===true)return false;
    const resolved=aircraft.type||aircraft.icaoType;if(!resolved)return false;
    $('type').value=resolved;formFieldSources.type='aircraft-db';calcEntry();saveEntryDraft();return true;
...
document.addEventListener('DOMContentLoaded',async()=>{
  ...
  $('logbookAddBtn').addEventListener('click',()=>{setEntryReturnContext(null);try{removeBusinessValue(ENTRY_DRAFT_KEY)}catch{};resetEntry(false);show('addView')});
  ...
  if('serviceWorker' in navigator){navigator.serviceWorker.register('./sw-11.9.js').catch(err=>console.warn('Offline cache unavailable',err));}
