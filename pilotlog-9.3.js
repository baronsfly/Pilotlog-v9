...
'use strict';
const VERSION='9.3';
// Legacy-named keys below are compatibility view identifiers. They no longer represent separate operational databases.
...
const BUSINESS_ARRAY_KEYS=new Set([TRIPS_KEY,EXPIRY_KEY,SYNC_LEDGER_KEY,ENTRY_DRAFTS_KEY,CLOUD_TOMBSTONES_KEY]);
...
function normalizeOperationalDatabase(value){
  ...
  return{...db,records:Array.isArray(db.records)?db.records:[],values:db.values&&typeof db.values==='object'?db.values:{},updatedAt:db.updatedAt||new Date().toISOString()};
}
...
function legacyLocalFlights(){const v=legacyLocalJson(FLIGHTS_KEY,[]);return Array.isArray(v)?v:[]}
...
function stripRosterViewFields(row){const{std,sta,status,completedEntryId,...rest}=row||{};return rest}
function stripDutyViewFields(row){const{report,end,minutes,notes,...rest}=row||{};return rest}
function unifiedFlightMatch(a,b){
  if(!a||!b||rawCanonicalType(a.dutyType)!=='Flight')return false;
  if(a.id&&b.id&&a.id===b.id)return true;
  if(a.legacyRosterId&&a.legacyRosterId===b.id)return true;
  if(a.rosterItemId&&a.rosterItemId===b.id)return true;
  if(a.aerolineKey&&b.aerolineKey&&a.aerolineKey===b.aerolineKey)return true;
  if(a.rosterAerolineKey&&b.aerolineKey&&a.rosterAerolineKey===b.aerolineKey)return true;
...
function migrateSnapshotToOperationalDatabase(snapshot={}){
  ...
  for(const source of (Array.isArray(snapshot.flights)?snapshot.flights:[])){
    const dutyType=rawCanonicalType(source.dutyType||'Flight'),flight=dutyType==='Flight';
    records.push({...source,id:source.id||makeId(),recordKind:'activity',dutyType,entryVisible:true,rosterVisible:!!source.aerolineRosterPlanned&&!source.rosterHidden,dutyVisible:false,activityStatus:flight?'done':(source.locked?'done':(source.status||'planned'))});
  }
  for(const source of (Array.isArray(snapshot.roster)?snapshot.roster:[])){
    const roster={...stripRosterViewFields(source),dutyType:'Flight',schedOut:source.std||source.schedOut||'',schedIn:source.sta||source.schedIn||'',rosterVisible:true,recordKind:'activity'};
    let i=records.findIndex(x=>x.id===source.completedEntryId||x.rosterItemId===source.id||unifiedFlightMatch(x,source));
    if(i>=0){
      const existing=records[i],operationalKeys=['date','flightNo','dep','arr','type','reg','schedOut','schedIn','out','off','on','in','block','flight','credit','role','seatPosition','instructionType','night','ifr','pf','approachType','picName','sicName','soName','instructorName','remarks','dayTakeoffs','nightTakeoffs','dayLandings','nightLandings','onDuty','offDuty','totalDuty','sectors','locked','manualOverride'];
      const merged={...existing,...roster,id:existing.id,legacyRosterId:source.id!==existing.id?source.id:(existing.legacyRosterId||''),rosterItemId:existing.id,rosterAerolineKey:source.aerolineKey||existing.rosterAerolineKey||'',entryVisible:!!existing.entryVisible,rosterVisible:true,dutyVisible:!!existing.dutyVisible,activityStatus:existing.entryVisible?'done':(source.status||'planned')};
      for(const key of operationalKeys)if(existing[key]!==undefined&&existing[key]!==null&&existing[key]!=='')merged[key]=existing[key];
      records[i]=merged;
    }else records.push({...roster,id:source.id||makeId(),entryVisible:false,dutyVisible:false,activityStatus:source.status||'planned',rosterItemId:source.id||'',rosterAerolineKey:source.aerolineKey||''});
  }
  for(const source of (Array.isArray(snapshot.duties)?snapshot.duties:[])){
    const dutyType=rawCanonicalType(source.type||source.dutyType||'Duty','Duty'),onDuty=source.report||source.onDuty||'',offDuty=source.end||source.offDuty||'';
    const i=records.findIndex(x=>x.date===source.date&&rawCanonicalType(x.dutyType,'Duty')===dutyType&&(x.onDuty||'')===onDuty&&(x.offDuty||'')===offDuty);
    const duty={...stripDutyViewFields(source),id:source.id||makeId(),recordKind:'activity',dutyType,onDuty,offDuty,remarks:source.notes||source.remarks||'',dutyMinutes:Number(source.minutes)||timeSpanMins(onDuty,offDuty),dutyVisible:true,rosterVisible:source.aerolineRosterPlanned!==false,activityStatus:source.status||'planned'};
    if(i>=0)records[i]={...duty,...records[i],dutyVisible:true,rosterVisible:records[i].rosterVisible||duty.rosterVisible};else records.push({...duty,entryVisible:false});
  }
  db.records=records;
  db.values={
...
async function saveFlightsDurable(rows){
  save(FLIGHTS_KEY,Array.isArray(rows)?rows:[]);
  await flushFlightStore();return true;
}
function entryViewRows(){return(operationalDatabase?.records||[]).filter(x=>x?.recordKind==='activity'&&x.entryVisible!==false&&x.entryVisible).map(x=>({...x}))}
function rosterViewRows(){return(operationalDatabase?.records||[]).filter(x=>x?.recordKind==='activity'&&x.rosterVisible&&rawCanonicalType(x.dutyType)==='Flight').map(x=>({...x,std:x.schedOut||'',sta:x.schedIn||'',status:x.activityStatus||'planned',completedEntryId:x.entryVisible?x.id:''}))}
function dutyViewRows(){return(operationalDatabase?.records||[]).filter(x=>x?.recordKind==='activity'&&x.dutyVisible).map(x=>({...x,type:x.dutyType||'Duty',report:x.onDuty||'',end:x.offDuty||'',minutes:Number(x.dutyMinutes)||timeSpanMins(x.onDuty||'',x.offDuty||''),notes:x.remarks||''}))}
function databaseValue(key,def){const v=operationalDatabase?.values?.[key];return v===undefined?def:v}
const load=k=>{if(k===FLIGHTS_KEY)return entryViewRows();if(k===ROSTER_KEY)return rosterViewRows();if(k===DUTY_KEY)return dutyViewRows();if(BUSINESS_KEYS.has(k)){const v=databaseValue(k,[]);return Array.isArray(v)?v:[]}try{return JSON.parse(localStorage.getItem(k)||'[]')}catch{return[]}};
...
function findRecordForEntry(row,records){return records.find(x=>x.id===row.id)||(row.rosterItemId?records.find(x=>x.id===row.rosterItemId||x.legacyRosterId===row.rosterItemId):null)||(row.rosterAerolineKey?records.find(x=>x.aerolineKey===row.rosterAerolineKey||x.rosterAerolineKey===row.rosterAerolineKey):null)||records.find(x=>unifiedFlightMatch(x,row))}
function saveEntryView(rows){
  const records=operationalDatabase.records||[],incoming=Array.isArray(rows)?rows:[],incomingIds=new Set(incoming.map(x=>x?.id).filter(Boolean)),kept=[];
  for(const record of records){
    if(!record?.entryVisible){kept.push(record);continue}
    if(incomingIds.has(record.id)){kept.push(record);continue}
    if(record.rosterVisible||record.dutyVisible)kept.push({...record,entryVisible:false,activityStatus:record.rosterVisible?'planned':record.activityStatus});
  }
  for(const source of incoming){
    if(!source)continue;
    let existing=findRecordForEntry(source,records)||kept.find(x=>x.id===source.id),id=existing?.id||source.id||makeId();
    const clean=stripRosterViewFields(stripDutyViewFields(source)),dutyType=rawCanonicalType(source.dutyType||existing?.dutyType||'Flight');
    const merged={...existing,...clean,id,recordKind:'activity',dutyType,entryVisible:true,rosterVisible:!!existing?.rosterVisible||!!source.rosterVisible||!!source.aerolineRosterPlanned,dutyVisible:!!existing?.dutyVisible};
    if(merged.rosterVisible&&dutyType==='Flight'){merged.activityStatus='done';merged.rosterItemId=id;merged.completedEntryId=id}
    const pos=kept.findIndex(x=>x.id===id);if(pos>=0)kept[pos]=merged;else kept.push(merged);
  }
  operationalDatabase.records=kept;return touchOperationalDatabase();
}
...
function saveRosterView(rows){
  const records=operationalDatabase.records||[],incoming=Array.isArray(rows)?rows:[],matched=new Set(),result=[];
  for(const source of incoming){
    if(!source)continue;
    const existing=records.find(x=>x.id===source.id||x.legacyRosterId===source.id||unifiedFlightMatch(x,source));
    const clean=stripRosterViewFields(source),candidate={...clean,dutyType:'Flight',schedOut:source.std||source.schedOut||'',schedIn:source.sta||source.schedIn||'',recordKind:'activity',rosterVisible:true};
    const id=existing?.id||source.id||makeId(),merged={...existing,...candidate,id,entryVisible:!!existing?.entryVisible,dutyVisible:!!existing?.dutyVisible,activityStatus:existing?.entryVisible?'done':(source.status||existing?.activityStatus||'planned'),legacyRosterId:source.id&&source.id!==id?source.id:(existing?.legacyRosterId||''),rosterItemId:id,rosterAerolineKey:source.aerolineKey||existing?.rosterAerolineKey||''};
    if(existing?.entryVisible)for(const key of AUTHORITATIVE_ENTRY_FIELDS)if(existing[key]!==undefined&&existing[key]!==null&&existing[key]!=='')merged[key]=existing[key];
    result.push(merged);if(existing)matched.add(existing.id);
  }
  for(const record of records){
    if(matched.has(record.id))continue;
    if(record.rosterVisible&&rawCanonicalType(record.dutyType)==='Flight'){
      if(record.entryVisible||record.dutyVisible)result.push({...record,rosterVisible:false});
    }else result.push(record);
  }
  operationalDatabase.records=result;return touchOperationalDatabase();
}
function saveDutyView(rows){
  const records=operationalDatabase.records||[],incoming=Array.isArray(rows)?rows:[],ids=new Set(incoming.map(x=>x?.id).filter(Boolean)),result=[];
  for(const record of records){if(record.dutyVisible&&!ids.has(record.id)){if(record.entryVisible||record.rosterVisible)result.push({...record,dutyVisible:false})}else result.push(record)}
  for(const source of incoming){
    const i=result.findIndex(x=>x.id===source.id),existing=i>=0?result[i]:null,dutyType=rawCanonicalType(source.type||source.dutyType||'Duty','Duty');
    const merged={...existing,...stripDutyViewFields(source),id:existing?.id||source.id||makeId(),recordKind:'activity',dutyType,onDuty:source.report||source.onDuty||'',offDuty:source.end||source.offDuty||'',remarks:source.notes||source.remarks||'',dutyMinutes:Number(source.minutes)||timeSpanMins(source.report||source.onDuty||'',source.end||source.offDuty||''),dutyVisible:true,entryVisible:!!existing?.entryVisible,rosterVisible:existing?.rosterVisible!==false&&(!!existing?.rosterVisible||source.aerolineRosterPlanned!==false),activityStatus:source.status||existing?.activityStatus||'planned'};
    if(i>=0)result[i]=merged;else result.push(merged);
  }
  operationalDatabase.records=result;return touchOperationalDatabase();
}
...
function operationalDatabaseCounts(database=operationalDatabase){
  ...
  return{activities:records.length,entries:records.filter(x=>x.entryVisible).length,roster:records.filter(x=>x.rosterVisible&&rawCanonicalType(x.dutyType)==='Flight').length,duties:records.filter(x=>x.dutyVisible).length,trips:Array.isArray(values[TRIPS_KEY])?values[TRIPS_KEY].length:0,expiry:Array.isArray(values[EXPIRY_KEY])?values[EXPIRY_KEY].length:0};
}
...
function rosterPlanningEntriesForMonth(month){
  return(operationalDatabase?.records||[])
    .filter(r=>r?.recordKind==='activity'&&inMonth(r.date,month)&&((isFlight(r)&&r.rosterVisible)||(!isFlight(r)&&r.entryVisible&&String(r.source||'').toLowerCase()!=='logten')))
    .map(r=>isFlight(r)&&r.rosterVisible?(r.entryVisible?{...r,_payrollRosterSynthetic:false,_trainingAuthoritative:true}:{...r,onDuty:'',offDuty:'',_payrollRosterSynthetic:true,_trainingAuthoritative:false}):{...r,_payrollRosterSynthetic:false,_trainingAuthoritative:!!r.entryVisible})
...
function operationalEntriesForMonth(month){
  ...
  const entries=(operationalDatabase?.records||[]).filter(r=>r?.recordKind==='activity'&&r.entryVisible&&inMonth(r.date,month));
  const logten=entries.filter(f=>String(f.source||'').toLowerCase()==='logten');
  if(logten.length)return logten;
  return entries.filter(f=>!f.aerolineRosterPlanned);
}
function operationalEntriesForTrips(){
  const activities=(operationalDatabase?.records||[]).filter(r=>r?.recordKind==='activity'),byMonth=new Map();
  ...
  for(const month of [...byMonth.keys()].sort()){
    ...
    const hasRoster=rows.some(r=>(isFlight(r)&&r.rosterVisible)||(r.entryVisible&&isFlight(r)&&r.aerolineRosterPlanned)||(r.dutyVisible&&r.aerolineRosterPlanned));
    if(hasRoster){
      out.push(...rows
        .filter(r=>(isFlight(r)&&r.rosterVisible)||(!isFlight(r)&&r.entryVisible&&String(r.source||'').toLowerCase()!=='logten'))
        .map(r=>isFlight(r)&&r.rosterVisible?(r.entryVisible?{...r,_payrollRosterSynthetic:false,_trainingAuthoritative:true}:{...r,onDuty:'',offDuty:'',_payrollRosterSynthetic:true,_trainingAuthoritative:false}):{...r,_payrollRosterSynthetic:false,_trainingAuthoritative:!!r.entryVisible})
        .sort((a,b)=>entryChrono(a).localeCompare(entryChrono(b))));
    }else{
      const entries=rows.filter(r=>r.entryVisible),logten=entries.filter(f=>String(f.source||'').toLowerCase()==='logten');
      out.push(...(logten.length?logten:entries.filter(f=>!f.aerolineRosterPlanned)));
    }
  }
...
async function restoreFullBackupJsonFile(file){
  ...
  reconcileAllDuties();markLocalOnlyImport(`Full backup restored locally — ${counts.activities} unique activities`);await render();await renderRoster();renderDuty();renderTrips();renderPayroll();renderSettings();resetEntry(false);restoreEntryDraft();
...
async function renderPayroll(){
  ...
  const ex=monthExtras(month),p=PilotLogEngine.payroll(month);$('payArrears').value=ex.arrears||0;$('payTotalMain').textContent='…';$('payFxStatus').textContent=cur==='MAD'?'No FX conversion required.':'Loading FX…';
...
function applyLedgerToUnifiedRecords(records,ledger){
  ...
  for(const source of (records||[])){
    let row={...source};
    if(row.entryVisible&&effectiveCloudDeletion('flights',row,ledger))row.entryVisible=false;
    if(row.rosterVisible&&effectiveCloudDeletion('roster',row,ledger))row.rosterVisible=false;
    if(row.dutyVisible&&effectiveCloudDeletion('duties',row,ledger))row.dutyVisible=false;
    if(row.entryVisible||row.rosterVisible||row.dutyVisible)out.push(row);
  }
  return out;
}
function mergeUnifiedDatabases(localDatabase,remoteDatabase){
  ...
  for(const row of [...(remote.records||[]),...(local.records||[])]){
    ...
    map.set(key,older?{...older,...newer,entryVisible:!!(older.entryVisible||newer.entryVisible),rosterVisible:!!(older.rosterVisible||newer.rosterVisible),dutyVisible:!!(older.dutyVisible||newer.dutyVisible)}:newer);
  }
...
function show(id,options={}){
  ...
  if(id==='flightsView'){
    if(options.logbookEntryId){
      const ordered=load(FLIGHTS_KEY).filter(isCompletedLogbookEntry).filter(f=>logbookMatchesQuery(f,logbookSearchQuery)).sort((a,b)=>`${b.date}${b.onDuty||b.schedOut||b.out||''}`.localeCompare(`${a.date}${a.onDuty||a.schedOut||a.out||''}`));
...
function calculateDashboardDutyRows(startOffset,count){
  const today=new Date(),duties=load(DUTY_KEY),groups=rosterGroups();
  const rows=[];
...
let logbookSearchQuery='',logbookRenderLimit=400;
...
function renderEntriesSafe(){
  try{
    const all=load(FLIGHTS_KEY).filter(isCompletedLogbookEntry),creditContext=entriesByDate(all);
    ...
    if(target){
      ...
      target.innerHTML=flightHtml(visible,true,creditContext)+(remaining?`<div class="actions logbook-more-actions"><button class="secondary" type="button" data-load-more-logbook>Load older entries • ${remaining.toLocaleString('en-US')} remaining</button></div>`:'');
    }
...
let rosterCalendarMonth=monthNow(),rosterSelectedDate=today();
...
async function renderRoster(){
  ...
  const todayKey=isoDateLocal(new Date()),groups=rosterGroups(),month=rosterCalendarMonth||monthNow(),year=Number(month.slice(0,4)),dates=rosterCalendarDates(month),years=rosterCalendarYears(groups);
  const monthButtons=ROSTER_MONTH_NAMES.map((name,i)=>{const value=`${year}-${String(i+1).padStart(2,'0')}`;return`<button type="button" class="roster-month-chip${value===month?' active':''}" data-roster-month="${value}">${name}</button>`}).join('');
  ...
  box.innerHTML=`
    <div class="roster-calendar-shell" data-roster-calendar-swipe>
      <div class="roster-year-row"><select id="rosterYearSelect" aria-label="Roster year">${years.map(y=>`<option value="${y}"${y===year?' selected':''}>${y}</option>`).join('')}</select></div>
      <div class="roster-calendar-head">
        <button type="button" class="roster-calendar-arrow" data-roster-month-shift="-1" aria-label="Previous month">‹</button>
        <div><b>${esc(rosterMonthLabel(`${month}-01`))}</b><span>All dates in local time</span></div>
        <button type="button" class="roster-calendar-arrow" data-roster-month-shift="1" aria-label="Next month">›</button>
      </div>
      <div class="roster-calendar-actions"><button type="button" class="secondary" data-roster-today>Today</button></div>
      <div class="roster-month-strip" aria-label="Months">${monthButtons}</div>
      <div class="roster-weekdays">${['MON','TUE','WED','THU','FRI','SAT','SUN'].map(x=>`<span>${x}</span>`).join('')}</div>
      <div class="roster-calendar-grid">${cells}</div>
    </div>
...
function buildTotalsProjection(entries,now=new Date()){
  const model=buildTotalsModel(entries,now),flying=model.flying,y=now.getUTCFullYear(),m=now.getUTCMonth();
  ...
  const approachTypes=['CAT3','ILS','LOC','RNAV','RNAV AR','VOR','NDB','CRCL','VIS','GOA'],approaches=Object.fromEntries(approachTypes.map(x=>[x,0]));
  flying.forEach(f=>{const a=upper(f.approachType||'');if(approaches[a]!==undefined)approaches[a]++});
  return{...model,periods,approachTypes,approaches};
}
...
const PilotLogEngine=Object.freeze({
  ...
  dutySessions:(entries=null)=>buildDutySessions(entries||load(FLIGHTS_KEY)),
...
async function renderTotals(){
  const model=PilotLogEngine.totals(load(FLIGHTS_KEY));
  ...
  $('approachTotals').innerHTML=model.approachTypes.map(a=>`<div class="stat-row"><span>${esc(a)}</span><b>${model.approaches[a]}</b></div>`).join('');
...
function bindPilotLogSwipeGestures(){
  ...
  document.addEventListener('touchstart',e=>{
    ...
    else if(e.target.closest('[data-roster-calendar-swipe]')&&!e.target.closest('.roster-month-strip'))calendarStart=stamp;
  },{passive:true});
  document.addEventListener('touchend',e=>{
    ...
    if(calendarStart){
      const dx=p.clientX-calendarStart.x,dy=p.clientY-calendarStart.y,elapsed=Date.now()-calendarStart.at;calendarStart=null;
      if(Math.abs(dx)>=60&&Math.abs(dx)>Math.abs(dy)*1.25&&elapsed<1200){rosterCalendarMonth=rosterShiftMonth(rosterCalendarMonth,dx<0?1:-1);rosterSelectedDate='';renderRoster()}
    }
  },{passive:true});
}
...
document.addEventListener('DOMContentLoaded',async()=>{
  ...
  document.addEventListener('change',e=>{
    if(e.target.id==='rosterYearSelect'){
      rosterCalendarMonth=`${e.target.value}-${String(rosterCalendarMonth||monthNow()).slice(5,7)}`;rosterSelectedDate='';renderRoster();return;
    }
  ...
  document.addEventListener('click',async e=>{let b=e.target.closest('[data-load-more-logbook]');if(b){logbookRenderLimit+=LOGBOOK_RENDER_STEP;renderEntriesSafe();return}
    b=e.target.closest('[data-roster-month-shift]');if(b){rosterCalendarMonth=rosterShiftMonth(rosterCalendarMonth,Number(b.dataset.rosterMonthShift)||0);rosterSelectedDate='';await renderRoster();return}
    b=e.target.closest('[data-roster-month]');if(b){rosterCalendarMonth=b.dataset.rosterMonth;rosterSelectedDate='';await renderRoster();return}
    b=e.target.closest('[data-roster-today]');if(b){rosterSelectedDate=today();rosterCalendarMonth=rosterSelectedDate.slice(0,7);await renderRoster();scrollRosterAgendaIntoView();return}
    b=e.target.closest('[data-roster-date]');if(b){const date=b.dataset.rosterDate;rosterSelectedDate=rosterSelectedDate===date?'':date;if(rosterSelectedDate)rosterCalendarMonth=date.slice(0,7);await renderRoster();if(rosterSelectedDate)scrollRosterAgendaIntoView();return}
    b=e.target.closest('[data-delete-flight]');if(b){if(!confirm('Confirm to delete this log entry?'))return;const rows=load(FLIGHTS_KEY),victim=rows.find(f=>f.id===b.dataset.deleteFlight);markCloudDeleted('flights',victim||b.dataset.deleteFlight);if(victim)markRosterSectorIncompleteForEntry(victim);save(FLIGHTS_KEY,rows.filter(f=>f.id!==b.dataset.deleteFlight));reconcileAllDuties();await render();scheduleAutoSync('delete-flight');return}
