...
function migrateSnapshotToOperationalDatabase(snapshot={}){
  ...
  for(const source of (Array.isArray(snapshot.roster)?snapshot.roster:[])){
    ...
    const matched=records.find(x=>x.id===source.completedEntryId||x.rosterItemId===source.id)||uniqueUnifiedFlightMatch(records,roster);
    let i=matched?records.indexOf(matched):-1;
    if(i>=0){
...
function saveRosterView(rows){
  ...
  for(const source of incoming){
    if(!source)continue;
    const existing=records.find(x=>x.id===source.id||x.legacyRosterId===source.id)||uniqueUnifiedFlightMatch(records,source);
...
function mergeOperationalFlightPair(entry,roster){
  const merged={...roster,...entry,id:entry.id,recordKind:'activity',dutyType:'Flight',entryVisible:true,rosterVisible:true,dutyVisible:!!entry.dutyVisible,activityStatus:'done',legacyRosterId:roster.id!==entry.id?roster.id:(entry.legacyRosterId||''),rosterItemId:entry.id,rosterAerolineKey:roster.aerolineKey||entry.rosterAerolineKey||''};
  for(const [key,value] of Object.entries(roster))if(blankOperationalValue(merged[key])&&!blankOperationalValue(value))merged[key]=value;
  return merged;
}
function reconcileOperationalFlightIdentities(){
  ...
  const reviews=[];
  for(const roster of [...records]){
    if(!roster?.rosterVisible||roster.entryVisible||!isFlight(roster))continue;
    const candidates=records.filter(entry=>entry!==roster&&entry?.entryVisible&&isFlight(entry)&&scheduledFlightIdentityMatch(entry,roster));
    if(candidates.length===1){
      const entry=candidates[0],next=mergeOperationalFlightPair(entry,roster),index=records.indexOf(entry);
      records[index]=next;records=records.filter(row=>row!==roster);merged++;continue;
    }
    if(candidates.length>1)reviews.push({kind:'ambiguous-flight-link',date:roster.date,flightNo:roster.flightNo||'',dep:roster.dep||'',arr:roster.arr||'',schedOut:roster.schedOut||roster.std||'',schedIn:roster.schedIn||roster.sta||'',candidateEntryIds:candidates.map(row=>row.id),createdAt:new Date().toISOString()});
  }
  ...
  const key=row=>[row.kind,row.date,row.flightNo,row.dep,row.arr,row.schedOut,row.schedIn,(row.candidateEntryIds||[]).join(',')].join('|');
  const combined=[...prior.filter(row=>row?.kind!=='ambiguous-flight-link'),...reviews],unique=[...new Map(combined.map(row=>[key(row),row])).values()];
  if(merged||JSON.stringify(prior)!==JSON.stringify(unique)){operationalDatabase.records=records;operationalDatabase.values[RECONCILIATION_REVIEW_KEY]=unique;touchOperationalDatabase()}
  return{merged,reviews:reviews.length};
}
...
function setEntryTypeUI(){
  ...
  if(sim){
    ...
    ['breakdownTitle','breakdownGrid','nightStatus'].forEach(id=>$(id)?.classList.add('hidden'));
    $('creditDisplay').readOnly=false;syncSimulatorDutySchedule();if(!$('editId').value)$('creditDisplay').value=fmt(Math.round(Number(paySettings().simCredit||0)*60));
    renderEntryDraftShelf();return;
  }
...
function calcEntry(){
  ...
  else if(dutyType==='Simulator')$('calcPreview').textContent=`Simulator ${fmt(c.simulatorTime)} • Credit ${fmt(c.credit)} • report ${$('schedOut').value||'--:--'} / end duty ${$('schedIn').value||'--:--'}`;
...
function rosterPlanningEntriesForMonth(month){
  return(operationalDatabase?.records||[])
    .filter(r=>r?.recordKind==='activity'&&inMonth(r.date,month)&&((isFlight(r)&&(r.rosterVisible||r.entryVisible))||(!isFlight(r)&&r.entryVisible&&String(r.source||'').toLowerCase()!=='logten')))
