...
function crewCarryData(entry){
  ...
  return {...crew,cabinCrew:[crew.attendant1Name,crew.attendant2Name,crew.attendant3Name],observerCrew:Array.isArray(entry.observerCrew)?[...entry.observerCrew]:[],studentName:entry.studentName||'',instruction:entry.instruction===true,examiner:entry.examiner===true};
}
function formCrewSnapshot(){
  const crew=crewCarryData(coreActivityRows().find(f=>f.id===$('editId')?.value)||{});
  CREW_NAME_FIELDS.forEach(key=>{crew[key]=upper($(key)?.value||'')});
  return {...crew,cabinCrew:[crew.attendant1Name,crew.attendant2Name,crew.attendant3Name],instruction:!!$('instructionCheck')?.checked,examiner:!!$('examinerCheck')?.checked};
}
...
document.addEventListener('DOMContentLoaded',async()=>{
  ...
  if('serviceWorker' in navigator){navigator.serviceWorker.register('./sw-13.7.js').catch(err=>console.warn('Offline cache unavailable',err));}
