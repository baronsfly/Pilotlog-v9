...
const CORE_106_SIM_CLASS_REPAIR_KEY='pilotlog10_6_sim_instruction_repair_v1';
async function repairLogTenSimulatorInstruction106(){
  if(localStorage.getItem(CORE_106_SIM_CLASS_REPAIR_KEY)==='done')return{changed:0};
  let changed=0;
  const repaired=coreActivityRows().map(row=>{
    const r={...row};
  ...
  localStorage.setItem(CORE_106_SIM_CLASS_REPAIR_KEY,'done');
  return{changed};
}
...
document.addEventListener('DOMContentLoaded',async()=>{
  ...
  await repairLogTenSimulatorInstruction106();
  ...
  if('serviceWorker' in navigator){navigator.serviceWorker.register('./sw-10.6.js').catch(err=>console.warn('Offline cache unavailable',err));}
