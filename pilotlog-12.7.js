...
async function renderPayroll(){
  ...
  if(regime==='MAC'){
    ...
    $('payBreakdown').innerHTML=[
      ['Regime','MAC',0],['Fix Salary','',p.fixed],['Credit H',fmt(p.creditMins),p.flightPay],['NF CH',String(p.nonFlightCreditCount),fmt(p.nonFlightCreditMins),'credit-info'],['Training sectors',String(p.training),p.trainingPay],['Layover (non-taxable)',fmt(p.layMins),p.layoverPay],['Simulator allowance',String(p.sims),p.simPay],['Call from Day OFF',String(p.dayOffCount),p.dayOffPay],['Arrears / adjustments (taxable, not protected)','',p.arrears]
    ].map(([n,q,v,kind],i)=>`<div class="pay-breakdown-row ${i%2?'pay-breakdown-alt':''}"><span class="pay-breakdown-label">${esc(n)}</span><span class="pay-breakdown-qty">${esc(q)}</span><b class="money pay-breakdown-amount">${kind==='credit-info'?esc(v):(n==='Regime'?'—':money(displayValue(v))+' '+unit)}</b></div>`).join('');
  }else{
...
function buildTotalsProjection(entries,now=new Date()){
  ...
  const recorded=Object.keys(approaches).filter(x=>upper(x)!=='AUTOLAND'),approachTypes=[...standardApproachTypes.filter(x=>recorded.includes(x)),...recorded.filter(x=>!standardApproachTypes.includes(x)).sort()];
...
async function renderTotals(){
  ...
  const recorded=Object.keys(approaches).filter(x=>upper(x)!=='AUTOLAND');
...
document.addEventListener('DOMContentLoaded',async()=>{
  ...
  if('serviceWorker' in navigator){navigator.serviceWorker.register('./sw-12.7.js').catch(err=>console.warn('Offline cache unavailable',err));}
