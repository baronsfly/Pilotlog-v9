...
function calculateLogbookStats(rows,total){
  const all=rows||[],flights=all.filter(isFlight),sims=all.filter(isSim);
  return{entries:all.length,total:Number(total)||0,sectors:flights.length,block:sum(flights,f=>Number(f.block)||0),credit:sum(all,f=>creditMinsForEntry(f,all)),simulators:sims.length};
}
