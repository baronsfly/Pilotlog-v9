...
async function renderTotals(){
  ...
  $('approachTotals').innerHTML=[
    ...
    ...[['CAT3','CAT III'],['ILS','CAT I'],['VOR','VOR'],['VIS','Visual'],['GOA','Go Arounds'],['RNAV','RNAV'],['LOC','LOC'],['NDB','NDB'],['CRCL','CIRC']].map(([key,label])=>`<div class="stat-row"><span>${label}</span><b>${key==='GOA'?sum(totalRows,f=>Number(f.goArounds)||0):(model.approaches[key]??0)}</b></div>`),
    ...model.approachTypes.filter(a=>!['CAT3','ILS','VOR','VIS','GOA','RNAV','LOC','NDB','CRCL'].includes(a)).map(a=>`<div class="stat-row"><span>${esc(a)}</span><b>${model.approaches[a]}</b></div>`)
  ].join('');
...
function saveEntryDraft(){
  ...
  try{
    ...
    const duty=controlledDuty(existing);if(duty){if(!f.timeOverrides.offDuty)f.offDuty=duty.control.state==='OPEN'?'':hhmm(new Date(duty.control.end));if(!f.timeOverrides.onDuty)f.onDuty=hhmm(new Date(duty.control.start));if(!f.timeOverrides.totalDuty)f.totalDuty=duty.control.state==='CLOSED'&&existing.id===duty.root.id?duty.control.minutes:null}
...
function loadEntryToForm(f,options={}){
  ...
  const duty=controlledDuty(f);if(duty){if(!formTimeOverrides.onDuty)$('onDuty').value=hhmm(new Date(duty.control.start));if(!formTimeOverrides.offDuty)$('offDuty').value=duty.control.state==='CLOSED'?hhmm(new Date(duty.control.end)):''}
...
document.addEventListener('DOMContentLoaded',async()=>{
  ...
  if('serviceWorker' in navigator){navigator.serviceWorker.register('./sw-13.5.js').catch(err=>console.warn('Offline cache unavailable',err));}
