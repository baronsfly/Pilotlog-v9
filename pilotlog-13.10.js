...
function carryAircraftToAerolineReturn(entry){
  ...
  if(!entry.date||!(entry.out||entry.schedOut)||!entry.dep||!entry.arr)return entry;
  const earlier=coreActivityRows().filter(f=>f.id!==entry.id&&isFlight(f)&&f.source==='aeroline'&&f.date&&(f.out||f.schedOut)&&
    (entry.aerolineBlockId?f.aerolineBlockId===entry.aerolineBlockId:!f.aerolineBlockId&&f.date===entry.date)&&entryChrono(f)<entryChrono(entry))
    .sort((a,b)=>entryChrono(b).localeCompare(entryChrono(a)));
  const previous=earlier[0];
  if(!previous||earlier[1]&&entryChrono(earlier[1])===entryChrono(previous)||upper(previous.dep)!==upper(entry.arr)||upper(previous.arr)!==upper(entry.dep))return entry;
  ...
  if(upper(entry.type)&&upper(previous.type)&&upper(entry.type)!==upper(previous.type))return entry;
...
document.addEventListener('DOMContentLoaded',async()=>{
  ...
  if('serviceWorker' in navigator){navigator.serviceWorker.register('./sw-13.10.js').catch(err=>console.warn('Offline cache unavailable',err));}
