...
function sourceFieldMerge(existing,incoming){
  ...
  // Lock protects user editing only. It must never block a LogTen -> LogTen source repair/re-import.
  const protectedAll=!!existing?.locked&&!(existingIsLogTen&&incomingIsLogTen);
  ...
  for(const [key,value] of Object.entries(incoming||{})){
    ...
    // LogTen is copy-only: on re-import preserve the source value exactly, including 0, blank and NULL.
    if(existingIsLogTen&&incomingIsLogTen){out[key]=value;continue}
...
function timeGroupFinalValue(entry,key,allEntries=null){
  ...
  // LogTen fields are copy-only. Auto Sync must never backfill/derive a missing LogTen time field.
  // Credit and Total Duty remain operational PilotLog calculations, not LogTen logbook authority fields.
  if(String(entry?.source||'').toLowerCase()==='logten'&&!['credit','totalDuty'].includes(key))return entryHasTimeValue(entry,key)?numericTime(entry[key]):0;
...
/* Credit rules */
const PAY_DEFAULTS={joinDate:'2014-04-01',fixedSalary:59320,base:46500,allowance:8000,transport:2500,pos:4000,telephone:500,uniform:300,meal:520,deduction:-3000,seniority2:10,seniority12:15,t1Max:25,t1Rate:170,t2Max:50,t2Rate:370,t3Max:75,t3Rate:665,t4Rate:1400,trainingRate:500,layoverRate:25,simAllowance:1000,dayOffRate:1000};
function paySettings(){
  const saved=loadObject(PAY_SETTINGS_KEY,{}),st={...PAY_DEFAULTS,...saved};
  if(!Object.prototype.hasOwnProperty.call(saved,'fixedSalary'))st.fixedSalary=Number(st.base||0)+Number(st.allowance||0)+Number(st.transport||0)+Number(st.pos||0)+Number(st.telephone||0)+Number(st.uniform||0)+Number(st.meal||0)+Number(st.deduction||0);
  return st;
}
...
function setEntryTypeUI(){
  ...
  $('flightForm').classList.toggle('roster-duty-column-layout',sim||positioning||ground||standby);
...
function calculatePayrollProjection(month){
  const st=paySettings(),
    ...
    seniority=Object.prototype.hasOwnProperty.call(st,'fixedSalary')?0:Number(st.base||0)*seniorPct/100,
    fixed=Object.prototype.hasOwnProperty.call(st,'fixedSalary')?Number(st.fixedSalary||0):Number(st.base||0)+Number(st.allowance||0)+Number(st.transport||0)+seniority+Number(st.pos||0)+Number(st.telephone||0)+Number(st.uniform||0)+Number(st.meal||0)+Number(st.deduction||0),
...
const PAY_MAP={setJoinDate:'joinDate',setFixedSalary:'fixedSalary',setT1Max:'t1Max',setT1Rate:'t1Rate',setT2Max:'t2Max',setT2Rate:'t2Rate',setT3Max:'t3Max',setT3Rate:'t3Rate',setT4Rate:'t4Rate',setTrainingRate:'trainingRate',setLayoverRate:'layoverRate',setSimAllowance:'simAllowance',setDayOffRate:'dayOffRate'};
function fillPaySettings(){const st=paySettings();Object.entries(PAY_MAP).forEach(([id,k])=>{if($(id))$(id).value=st[k]})}
function readPaySettings(){const st={...paySettings()};Object.entries(PAY_MAP).forEach(([id,k])=>st[k]=k==='joinDate'?$(id).value:Number($(id).value||0));return st}
...
function expiryRowHtml(x){
  ...
  const photo=x.photoData?`<button class="link-button" data-view-expiry-photo="${x.id}">VIEW PHOTO</button>`:'';
  return `<div class="expiry-row expiry-${s.state}${x.locked?' expiry-locked':''} expiry-row-open" data-open-expiry="${x.id}" role="button" tabindex="0">
...
function openExpiryEditor(category,item=null){
  ...
  $('expiryForm').querySelectorAll('input:not([type=hidden]),select,textarea').forEach(el=>el.disabled=locked);
  if($('expiryEditorLockBtn'))$('expiryEditorLockBtn').textContent=locked?'🔓 UNLOCK':'🔒 LOCK';
  if($('expiryEditorDeleteBtn'))$('expiryEditorDeleteBtn').classList.toggle('hidden',!item);
...
function saveExpiryFromForm(options={}){
  ...
  if(options.close===true)$('expiryEditorWrap').classList.add('hidden');
  renderExpiry();
...
function bindExpiryEvents(){
  ...
  const expiryAutoSave=()=>{if($('expiryEditorWrap').classList.contains('hidden'))return;const id=$('expiryEditId').value;if(id&&load(EXPIRY_KEY).find(x=>x.id===id)?.locked)return;try{saveExpiryFromForm({partial:true})}catch(e){console.warn('Expiry autosave skipped',e)}};
  $('expiryForm').addEventListener('change',expiryAutoSave);
  $('expiryForm').addEventListener('input',e=>{if(e.target.matches('input:not([type=file]),textarea')){clearTimeout(expiryAutoSave._t);expiryAutoSave._t=setTimeout(expiryAutoSave,250)}});
  $('expiryEditorLockBtn')?.addEventListener('click',()=>{const id=$('expiryEditId').value;if(!id)return;const rows=load(EXPIRY_KEY),x=rows.find(v=>v.id===id);if(!x)return;const next=!x.locked;if(!confirm(next?'LOCK THIS EXPIRY ITEM?':'UNLOCK THIS EXPIRY ITEM?'))return;x.locked=next;x._updatedAt=new Date().toISOString();x._syncRev=Number(x._syncRev||0)+1;save(EXPIRY_KEY,rows);markCloudEdited('expiry',x,next?'lock':'unlock');renderExpiry();openExpiryEditor(x.category,x);scheduleAutoSync(next?'expiry-lock':'expiry-unlock')});
  $('expiryEditorDeleteBtn')?.addEventListener('click',()=>{const id=$('expiryEditId').value;if(!id)return;const rows=load(EXPIRY_KEY),x=rows.find(v=>v.id===id);if(!x)return;if(x.locked)return alert('THIS EXPIRY ITEM IS LOCKED. UNLOCK IT BEFORE DELETING.');if(!confirm('DELETE THIS EXPIRY RECORD?'))return;markCloudDeleted('expiry',x);save(EXPIRY_KEY,rows.filter(v=>v.id!==id));$('expiryEditorWrap').classList.add('hidden');resetExpiryEditor();renderExpiry();scheduleAutoSync('expiry-delete')});
  ...
  document.addEventListener('click',e=>{
    const open=e.target.closest('[data-open-expiry]');
    if(open&&!e.target.closest('[data-view-expiry-photo]')){const x=load(EXPIRY_KEY).find(v=>v.id===open.dataset.openExpiry);if(x)openExpiryEditor(x.category,x);return}
...
function buildAircraftTypeBreakdown(flying,simulators){
  const familyCode=code=>['A319','A320'].includes(upper(code))?'A320 Family':code;
  ...
  flying.forEach(f=>{const rawCode=aircraftIcaoType(f);if(!rawCode)return;const code=familyCode(rawCode),r=ensure(code),mins=totalFlightMins(f);r.variants.add(upper(f.type||f.aircraftType||rawCode));r.flight+=mins;r.pic+=picMins(f);r.sic+=sicMins(f);r.instruction+=flightInstrMins(f)});
  ...
  simulators.forEach(f=>{const rawCode=aircraftIcaoType(f);if(!rawCode)return;const code=familyCode(rawCode);if(!rows.has(code))return;const r=rows.get(code);r.variants.add(upper(f.type||f.aircraftType||rawCode));r.simulator+=timeGroupFinalValue(f,'simulatorTime');r.simInstruction+=simInstrMins(f)});
...
document.addEventListener('DOMContentLoaded',async()=>{
  ...
  if('serviceWorker' in navigator){navigator.serviceWorker.register('./sw-12.6.js').catch(err=>console.warn('Offline cache unavailable',err));}
