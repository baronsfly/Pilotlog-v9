...
function mergeCoreActivities(a,b){
  ...
  const primaryAll=!!primary?.locked||primary?.source==='manual',secondaryAll=!!secondary?.locked||secondary?.source==='manual';
  ...
  for(const key of keys){
    ...
    if(sManual&&!pManual&&Object.prototype.hasOwnProperty.call(secondary,key))merged[key]=secondary[key];
    else if(!pManual&&!sManual&&blankOperationalValue(merged[key])&&!blankOperationalValue(secondary?.[key]))merged[key]=secondary[key];
  }
...
function mergeDuplicateEntries(a,b){
  ...
  const merged={...secondary,...primary},primaryAll=!!primary?.locked||primary?.source==='manual',secondaryAll=!!secondary?.locked||secondary?.source==='manual';
  for(const k of new Set([...Object.keys(primary||{}),...Object.keys(secondary||{})])){
    ...
    if(sManual&&!pManual&&Object.prototype.hasOwnProperty.call(secondary,k))merged[k]=secondary[k];
    else if(!pManual&&!sManual&&(merged[k]===undefined||merged[k]===null||merged[k]==='')&&secondary[k]!==undefined&&secondary[k]!==null&&secondary[k]!=='')merged[k]=secondary[k];
  }
  merged.manualFields={...(secondary?.manualFields||{}),...(primary?.manualFields||{})};
  merged.timeOverrides={...(secondary?.timeOverrides||{}),...(primary?.timeOverrides||{})};
  merged.fieldSources={...(secondary?.fieldSources||{}),...(primary?.fieldSources||{})};
      ...
      if(!seenTraining.has(key)){
        ...
        if(start.date){
          ...
          activities.push({
            ...
            schedOut:start.time,schedIn:end.time,schedBlock:timeSpanMins(start.time,end.time),onDuty:shiftTime(start.time,-60),offDuty:shiftTime(end.time,30),fieldSources:{schedOut:'aeroline',schedIn:'aeroline',schedBlock:'calculated',onDuty:'calculated',offDuty:'calculated'},out:'',off:'',on:'',in:'',block:null,
...
async function repairCore107SupplementalFromLegacy(){
  ...
  const repaired=coreActivityRows().map(row=>{
    ...
    for(const field of supplemental){
      ...
      if((r.locked||r.source==='manual'||entryManualField(r,field))&&hasCurrent)continue;
...
document.addEventListener('DOMContentLoaded',async()=>{
  ...
  if('serviceWorker' in navigator){navigator.serviceWorker.register('./sw-11.4.js').catch(err=>console.warn('Offline cache unavailable',err));}
