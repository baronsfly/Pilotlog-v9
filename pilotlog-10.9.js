...
function calculateActivityMetrics(f,allEntries=null){
  ...
  const timedBlock=timeSpanMins(f?.out||'',f?.in||'');
  const block=flight?timedBlock:sim?(f?.simBlockExplicit===true?Math.max(0,Number(f?.block)||0):(Math.max(0,Number(f?.block)||0)||timedBlock)):0;
  const flightMins=flight?timeSpanMins(f?.off||'',f?.on||''):0;
  const simulatorTime=sim?(f?.simulatorTimeExplicit===true?Math.max(0,Number(f?.simulatorTime)||0):(Math.max(0,Number(f?.simulatorTime)||0)||timedBlock)):0;
  const timedDuty=sim?timeSpanMins(f?.onDuty||'',f?.offDuty||''):0;
  const totalDuty=sim?(f?.simDutyExplicit===true?Math.max(0,Number(f?.totalDuty)||0):(Math.max(0,Number(f?.totalDuty)||0)||timedDuty)):Math.max(0,Number(f?.totalDuty)||0);
...
function formActivityInput(){
  ...
  return{id:$('editId')?.value||'',dutyType,date:$('date')?.value||'',flightNo:composeFlightNo($('flightNo')?.value||''),dep:upper($('dep')?.value||''),arr:upper($('arr')?.value||''),schedOut:$('schedOut')?.value||'',schedIn:$('schedIn')?.value||'',onDuty:$('onDuty')?.value||'',offDuty:$('offDuty')?.value||'',out:$('out')?.value||'',off:$('off')?.value||'',on:$('on')?.value||'',in:$('in')?.value||'',block:dutyType==='Simulator'?durMins($('blockDisplay')?.value||'0:00'):Number(existing?.block)||0,simBlockExplicit:dutyType==='Simulator',simulatorTime:dutyType==='Simulator'?durMins($('simulatorTimeDisplay')?.value||'0:00'):Number(existing?.simulatorTime)||0,simulatorTimeExplicit:dutyType==='Simulator',totalDuty:dutyType==='Simulator'?durMins($('totalDutyDisplay')?.value||'0:00'):Number(existing?.totalDuty)||0,simDutyExplicit:dutyType==='Simulator',credit:editableCredit?durMins($('creditDisplay')?.value||'0:00'):Number(existing?.credit)||0,simCreditExplicit:dutyType==='Simulator',groundCreditExplicit:dutyType==='Ground Course',aerolineTrainerType:existing?.aerolineTrainerType||''};
}
...
function setEntryTypeUI(){
  ...
  $('blockDisplay').readOnly=true;$('simulatorTimeDisplay').readOnly=true;$('totalDutyDisplay').readOnly=true;
  ...
  if(sim){
    const hide=['flightNo','dep','arr','schedOut','schedIn','schedBlock','off','on','role','seatPosition','night','ifr','pf','approachType','delayReason'];
    ...
    ['breakdownTitle','breakdownGrid','nightStatus'].forEach(id=>$(id)?.classList.add('hidden'));
    $('blockDisplay').readOnly=false;$('simulatorTimeDisplay').readOnly=false;$('totalDutyDisplay').readOnly=false;
...
function calcEntry(){
  ...
  $('schedBlockDisplay').value=fmt(c.schedBlock);if(dutyType!=='Simulator'||!$('blockDisplay').value)$('blockDisplay').value=fmt(c.block);$('creditDisplay').value=(dutyType==='Simulator'||dutyType==='DHP'||dutyType==='Ground Course')?$('creditDisplay').value:fmt(c.credit);
  const role=$('role').value,inst=$('instructionType').value;
  $('totalTimeDisplay').value=fmt(dutyType==='Flight'?c.block:0);if($('simulatorTimeDisplay')&&(dutyType!=='Simulator'||!$('simulatorTimeDisplay').value))$('simulatorTimeDisplay').value=fmt(c.simulatorTime);if(dutyType==='Simulator'&&$('totalDutyDisplay')&&!$('totalDutyDisplay').value)$('totalDutyDisplay').value=fmt(c.totalDuty);
...
function flightDutyWindow(f){
  ...
  if(!release&&isSim(f)&&Number(f.totalDuty)>0)release=new Date(report.getTime()+Math.max(0,Number(f.totalDuty))*60000);
...
function reconcileAllDuties(){
  ...
  fs.forEach(f=>{if(!(isSim(f)&&f.simDutyExplicit===true))f.totalDuty=0;f.sectors=0;if(isPositioning(f)){f.onDuty='';f.offDuty=''}else if(isFlight(f)){f.onDuty='';f.offDuty=''}});
  ...
  sessions.forEach(s=>{
    ...
    relevant.forEach(f=>{f.sectors=flights.length;if(!(isSim(f)&&f.simDutyExplicit===true))f.totalDuty=0});
    if(first)first.onDuty=s.report;
    if(last){last.offDuty=s.end;if(!(isSim(last)&&last.simDutyExplicit===true))last.totalDuty=s.minutes}
  });
...
function logTenMigrationEntry(raw){
  ...
  if(isSim(f)){
    ...
    /* LogTen simulator classification: only explicit SFI/SFE time makes this an instructor simulator. */
    if(Object.prototype.hasOwnProperty.call(f,'sfiSfe')){
      f.instructionType=durMins(f.sfiSfe)>1?'SFI/SFE Instruction Sim':'';
    }
  }else delete f.sim;
...
function expTotals(list){
  ...
  return{
    ...
    sim:sum(sims,f=>Number(f.simulatorTime)||Number(f.block)||0),simInstr:sum(sims,simInstrMins)
  };
}
...
function buildAircraftTypeBreakdown(flying,simulators){
  ...
  simulators.forEach(f=>{const code=aircraftIcaoType(f);if(!code||!rows.has(code))return;const r=rows.get(code);r.variants.add(upper(f.type||f.aircraftType||code));r.simulator+=Number(f.simulatorTime)||Number(f.block)||0;r.simInstruction+=simInstrMins(f)});
...
function buildTotalsModel(entries,now=new Date()){
  ...
  return{
    ...
    totalFlight:sum(flying,totalFlightMins),totalSimulator:sum(simulators,f=>Number(f.simulatorTime)||Number(f.block)||0),a320Flight:sum(a320Flights,totalFlightMins),a320Pic:sum(a320Flights,picMins),a320Sic:sum(a320Flights,sicMins),
    a320FlightInstruction:sum(a320Flights,flightInstrMins),a320SimInstruction:sum(a320Sims,simInstrMins),a320SimulatorTime:sum(a320Sims,f=>Number(f.simulatorTime)||Number(f.block)||0),
...
async function renderTotals(){
  ...
  const totalRows=coreRows.filter(f=>isCompletedLogbookEntry(f)||(isSim(f)&&simulatorCompleted(f)));
...
function loadEntryToForm(f,options={}){
  ...
  $('flightNo').value=flightNoInput(f.flightNo||'');$('reg').value=isSim(f)?upper(f.reg||''):aircraftIdInput(f.reg||'');$('simLocation').value=upper(f.location||(isSim(f)?f.dep:'')||'');$('creditDisplay').value=fmt(PilotLogEngine.credit({...f,dutyType:canonicalType}));$('sectorDisplay').value=String(f.sectors||'');$('blockDisplay').value=isSim(f)?fmt(Number(f.block)||Number(f.simulatorTime)||0):'';$('simulatorTimeDisplay').value=isSim(f)?fmt(Number(f.simulatorTime)||Number(f.block)||0):'';$('totalDutyDisplay').value=f.totalDuty?fmt(f.totalDuty):'';
...
function collectEntry(lockedOverride=null){
  ...
  return stamp({
    ...
    block:flight?c.block:sim?durMins($('blockDisplay')?.value||fmt(c.block)):0,flight:flight?c.flight:0,simulatorTime:sim?durMins($('simulatorTimeDisplay')?.value||fmt(c.simulatorTime)):0,totalDuty:sim?durMins($('totalDutyDisplay')?.value||fmt(c.totalDuty)):Number(existing?.totalDuty)||0,simBlockExplicit:sim?true:!!existing?.simBlockExplicit,simulatorTimeExplicit:sim?true:!!existing?.simulatorTimeExplicit,simDutyExplicit:sim?true:!!existing?.simDutyExplicit,credit:c.credit,simCreditExplicit:sim?true:!!existing?.simCreditExplicit,groundCreditExplicit:dutyType==='Ground Course'?true:!!existing?.groundCreditExplicit,
    ...
    pic:existing?.pic,sic:existing?.sic,purserName:existing?.purserName||'',cabinCrew:Array.isArray(existing?.cabinCrew)?existing.cabinCrew:existing?.cabinCrew,observerCrew:Array.isArray(existing?.observerCrew)?existing.observerCrew:existing?.observerCrew,studentName:existing?.studentName||'',
    pax:Object.prototype.hasOwnProperty.call(existing||{},'pax')?existing.pax:undefined,fuel:Object.prototype.hasOwnProperty.call(existing||{},'fuel')?existing.fuel:undefined,fuelAboard:existing?.fuelAboard,fuelBurned:existing?.fuelBurned,fuelMinimum:existing?.fuelMinimum,customNote:existing?.customNote,goArounds:existing?.goArounds,logtenCustom:existing?.logtenCustom,
    pf:sim?'no':($('pf')?.value||'no'),approachType:sim?'':upper($('approachType')?.value||''),approaches:sim?[]:(()=>{const selected=upper($('approachType')?.value||'');const priorType=upper(existing?.approachType||existing?.approaches?.[0]?.type||'');if(existing&&selected===priorType&&Array.isArray(existing.approaches))return existing.approaches;if(!selected)return[];return[{type:selected,quantity:1}]})(),delayMinutes,delayReason:delayMinutes?upper($('delayReason')?.value||''):'',remarks:$('remarks').value.trim(),
...
async function repairCore105FromLegacySources(){
  ...
  const recoveredEntries=legacyEntries.filter(Boolean).map(raw=>{
    ...
    if(isFlight(r)&&r.instructionType==='Flight Instruction'){
      r.trainingSector=true;
      if(!(Number(r.instructorFlightTime)>0)&&Number(r.block)>0)r.instructorFlightTime=Number(r.block);
    }
    return r;
  });
  ...
  const repaired=coreActivityRows().map(row=>{
    ...
    if(type==='Flight'&&(r.instructionType==='Flight Instruction'||r.trainingSector===true)){
...
async function repairLogTenSimulatorInstruction107(){
  if(localStorage.getItem(CORE_107_SIM_CLASS_REPAIR_KEY)==='done')return{changed:0};
  let changed=0;
  const repaired=coreActivityRows().map(row=>{
    const r={...row};
    if(!isSim(r)||String(r.source||'').toLowerCase()!=='logten')return r;
    /* The corrected migration package carries sfiSfe on every LogTen simulator row.
       Only values > 0:01 are instructor simulator time. Zero/blank is not instruction. */
    if(Object.prototype.hasOwnProperty.call(r,'sfiSfe')){
      const shouldInstruct=durMins(r.sfiSfe)>1;
      const desired=shouldInstruct?'SFI/SFE Instruction Sim':'';
      if((r.instructionType||'')!==desired){r.instructionType=desired;changed++}
    }
    return r;
  });
...
document.addEventListener('DOMContentLoaded',async()=>{
  ...
  ['blockDisplay','simulatorTimeDisplay','totalDutyDisplay'].forEach(id=>$(id)?.addEventListener('change',()=>{if(canonicalDutyType($('dutyTypeFlight')?.value||'Flight')!=='Simulator')return;$(id).value=fmt(durMins($(id).value||'0:00'));calcEntry();silentAutosaveExisting()}));
  ...
  if('serviceWorker' in navigator){navigator.serviceWorker.register('./sw-10.9.js').catch(err=>console.warn('Offline cache unavailable',err));}
