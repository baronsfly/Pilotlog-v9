...
function reconcileLogTenEntries(existingRows,rawEntries){
  const fs=[],duplicates=new Map(),removedIds=[],uidIndex=new Map(),dateIndex=new Map();let imported=0,updated=0,protectedMerged=0,conflicts=0;
  for(const row of existingRows){
    const uid=String(row.logtenUniqueId||'');
    if(uid&&duplicates.has(uid)){
      const i=duplicates.get(uid),prior=fs[i];
      for(const key of new Set([...Object.keys(prior.manualFields||{}),...Object.keys(row.manualFields||{}),...Object.keys(prior.timeOverrides||{}),...Object.keys(row.timeOverrides||{})])){
        if(entryManualField(prior,key)&&entryManualField(row,key)&&JSON.stringify(prior[key])!==JSON.stringify(row[key]))throw new Error(`Conflicting manual values for duplicate LogTen ID ${uid}, field ${key}. No entries were imported.`);
      }
      const merged=mergeCoreActivities(prior,row);
      for(const original of [prior,row])for(const key of Object.keys(original))if(entryManualField(original,key)){merged[key]=original[key];merged.fieldSources[key]='manual'}
      removedIds.push(merged.id===prior.id?row.id:prior.id);fs[i]=merged;
    }else{if(uid)duplicates.set(uid,fs.length);fs.push({...row})}
  }
  fs.forEach((x,i)=>{if(x.logtenUniqueId){const uid=String(x.logtenUniqueId);if(uidIndex.has(uid))throw new Error(`Duplicate existing LogTen ID: ${uid}. Resolve before importing.`);uidIndex.set(uid,i)}const d=String(x.date||'');if(!dateIndex.has(d))dateIndex.set(d,[]);dateIndex.get(d).push(i)});
  const seen=new Set();
  for(const raw of rawEntries){
    const incoming=logTenMigrationEntry(raw),uid=incoming.logtenUniqueId;
    if(!/^\d{4}-\d{2}-\d{2}$/.test(incoming.date))throw new Error('Invalid LogTen flight date. No entries were imported.');
    if(seen.has(uid))throw new Error(`Duplicate LogTen ID in source: ${uid}.`);seen.add(uid);
    let idx=uidIndex.has(uid)?uidIndex.get(uid):-1;
    if(idx<0){const matches=(dateIndex.get(incoming.date)||[]).filter(i=>logTenMigrationFallbackMatch(fs[i],incoming));
      if(matches.length>1)throw new Error(`Ambiguous LogTen match: ${incoming.date} ${incoming.flightNo||incoming.dutyType}. No entries were imported.`);
      if(matches.length===1)idx=matches[0];
    }
    if(idx>=0){fs[idx]=mergeLogTenMigration(fs[idx],incoming);updated++;const n=fs[idx].logtenImportConflicts.length;if(n){protectedMerged++;conflicts+=n}}
    else{delete incoming._logtenImportType;delete incoming._logtenLegacyKey;incoming._updatedAt=new Date().toISOString();idx=fs.length;fs.push(incoming);imported++;if(!dateIndex.has(incoming.date))dateIndex.set(incoming.date,[]);dateIndex.get(incoming.date).push(idx)}
    uidIndex.set(uid,idx);
  }
...
async function importLogTenMigrationPackage(file){
  ...
  removedIds.forEach(id=>markCloudDeleted('activities',id,{includeSource:false}));
