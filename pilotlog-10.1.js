...
function normalizeCoreActivityRecords(rows){
  ...
  for(const raw of (rows||[])){
    ...
    let match=out.find(x=>x.id===row.id);
    if(!match&&legacyActivityField(raw,'completed'))match=out.find(x=>x.id===legacyActivityField(raw,'completed'));
    if(!match&&legacyActivityField(raw,'rosterItem'))match=out.find(x=>x.id===legacyActivityField(raw,'rosterItem'));
    if(!match&&rawCanonicalType(row.dutyType)==='Flight')match=uniqueUnifiedFlightMatch(out.filter(x=>rawCanonicalType(x.dutyType)==='Flight'),row);
    if(match){const i=out.indexOf(match);out[i]=mergeCoreActivities(match,row)}else out.push(row);
  }
  return out;
}
function migrateSnapshotToOperationalDatabase(snapshot={}){
  ...
  for(const source of (Array.isArray(snapshot.roster)?snapshot.roster:[])){
    ...
    const matched=records.find(x=>x.id===legacyActivityField(source,'completed')||x.id===source.id)||uniqueUnifiedFlightMatch(records,roster);
    let i=matched?records.indexOf(matched):-1;
    if(i>=0){
      records[i]=mergeCoreActivities(records[i],roster);
    }else records.push(roster);
  }
...
function reconcileOperationalFlightIdentities(){
  ...
  for(const row of records.filter(x=>rawCanonicalType(x.dutyType)==='Flight')){const candidates=records.filter(x=>x!==row&&rawCanonicalType(x.dutyType)==='Flight'&&scheduledFlightIdentityMatch(x,row));if(candidates.length>1)reviews.push({kind:'ambiguous-flight-link',date:row.date,flightNo:row.flightNo||'',dep:row.dep||'',arr:row.arr||'',schedOut:row.schedOut||'',schedIn:row.schedIn||'',candidateEntryIds:candidates.map(x=>x.id),createdAt:new Date().toISOString()})}
...
function cleanupRosterDuplicates(){
  let activities=normalizeCoreActivityRecords(coreActivityRows()),removed=coreActivityRows().length-activities.length;
  let changed=true;
  while(changed){
    changed=false;
    outer:for(let i=0;i<activities.length;i++)for(let j=i+1;j<activities.length;j++){
      if(!rosterActivitySameIdentity(activities[i],activities[j]))continue;
      const merged=mergeCoreActivities(activities[i],activities[j]);
      const keepId=merged.id||activities[i].id||activities[j].id||makeActivityId();
      const removedId=keepId===activities[i].id?activities[j].id:activities[i].id;
      if(removedId)markCloudDeleted('activities',removedId,{includeSource:false});
      merged.id=keepId;activities[i]=merged;activities.splice(j,1);removed++;changed=true;break outer;
    }
  }
  if(removed){
...
function runRosterLogbookIntegrityCheck(){
  ...
  for(let i=0;i<flights.length;i++)for(let j=i+1;j<flights.length;j++)if(unifiedFlightMatch(flights[i],flights[j]))semanticDuplicates.push([flights[i],flights[j]]);
